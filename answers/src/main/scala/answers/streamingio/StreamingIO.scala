package answers.streamingio

import answers.iomonad.{IO,Monad,Free,unsafePerformIO}
...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    def runLog[O](src: Process[IO,O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO,O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req,recv) =>
            val next =
              try recv(Right(answers.iomonad.unsafePerformIO(req)(E)))
    ...
    import answers.iomonad.IO0.fahrenheitToCelsius
