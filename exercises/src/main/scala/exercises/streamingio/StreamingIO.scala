package exercises.streamingio

import exercises.iomonad.{IO,Monad,Free,unsafePerformIO}
...
object SimpleStreamTransducers {
  ...
  object Process {
    ...
    import exercises.iomonad.Monad
...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    def runLog[O](src: Process[IO,O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO,O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req,recv) =>
            val next =
              try recv(Right(exercises.iomonad.unsafePerformIO(req)(E)))
    ...
    import exercises.iomonad.IO0.fahrenheitToCelsius
